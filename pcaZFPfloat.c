...
gsl_matrix* eigenvectors = gsl_matrix_alloc(cols, cols);
	...
	//	unsigned char *bytes = SZ_compress(SZ_DOUBLE, delta, &outSize, 0, 0, 0,0, rows* cols);
		c_size+=zfp1D_floatcompress(delta,sizeN,EB);
...
//		printf("rows = %d, columns = %d \n",rows,cols);
		...
		//printf("rows = %d, columns = %d \n",rows,cols);
		//signed char *bytes_red = SZ_compress(SZ_DOUhBLE, delta, &outSize, 0, 0, 0, 0,rows* cols_red);
		c_size+=zfp1D_floatcompress(delta,rows* cols_red,EB);
		...
		c_size+=zfp1D_floatcompress(delta,cols* cols_red,EB);
