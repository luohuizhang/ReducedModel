...
gsl_matrix* pca(const gsl_matrix* data, gsl_matrix* diff,double threshold)
{
	/*
...
int main(int argc, char* argv[])
{
	...
	for(int round=0;round<Nrun;round++)
	{
		...
		red_data = pca(&mat.matrix,diff,Threshold);
		for(i=0;i<rows;i++)
			for(j=0;j<cols;j++)
				delta[j+i*cols]=gsl_matrix_get(diff,i,j);
		//printf("rows = %d, columns = %d \n",rows,cols);
		size_t outSize;
		unsigned char *bytes = SZ_compress(SZ_DOUBLE, delta, &outSize, 0, 0, 0, rows, cols);
		c_size+=outSize;
		...
		//printf("rows = %d, columns = %d \n",rows,cols);
		signed char *bytes_red = SZ_compress(SZ_DOUBLE, delta, &outSize, 0, 0, 0, rows, cols_red);
		c_size+=outSize;
