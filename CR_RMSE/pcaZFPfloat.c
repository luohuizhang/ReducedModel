...
int main(int argc, char* argv[])
{
	...
	for(int round=0;round<Nrun;round++)
	{
		...
		gsl_matrix* red_data1 = gsl_matrix_alloc(rows, cols_red);
		for(int EBi=0;EBi<31;EBi++){

			double EB=(max-min)*errorBound[EBi];
			//double EB=(max-min)*0.00001;
			for(i=0;i<rows;i++)
				for(j=0;j<cols;j++)
			...
			gsl_blas_dgemm(CblasNoTrans, CblasTrans, 1.0,  red_data1, &L_eigenvectors.matrix, 0.0, diff1);
			for(i = 0; i < rows; i++) {
