...
int main(int argc, char* argv[])
{
	...
	float *data =malloc(sizeN*sizeof(float));

	for(int round=0;round<Nrun;round++)
	{
		fread(data,sizeof(float),sizeN,fp);
		float max=-100000000;
		float min=100000000;
		for(i=0;i<sizeN;i++){
			if(data[i]>max)
		...
		float EB=(max-min)*ratioEB;
		sz.absErrBound = EB;
               // printf("EB=%lf\n",EB);

	SZ_Init_Params(&sz);
		size_t outSize;
		unsigned char *bytes = SZ_compress(SZ_FLOAT, data, &outSize, 0, 0, 0, 0,rows*cols);
		c_size+=outSize;
