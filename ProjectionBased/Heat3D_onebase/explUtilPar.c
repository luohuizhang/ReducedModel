...
void computeNext1(double*** x0, double*** x, int size_x, int size_y, int size_z, double dt, double hx, double hy, 
                 double hz, double* r, int me, int* xs, int* ys, int* zs, int* xe, int* ye, int* ze, double k0)
{
   /* Index variables */
   int i, j, k;
...
void processToMap1(int me, int *xs, int *xe, int *ys, int *ye, int *zs, int *ze, int xcell, 
...
void regen_linear(double*** x, double*** reduced_buffer, int ratio_x, int ratio_y, int ratio_z, int me, int* xs, int* xe, int* ys, int* ye, int* zs, int* ze, int xcell, int ycell, int zcell)
{
   ...
   for(k=0;k<zcell;k++)
     ...
     //if(x[xs[me]+ratio_x*i][ys[me]+j*ratio_y][zs[me]+k]-reduced_buffer[i][j][0]<-4)
//     printf("%f,%f\n", x[xs[me]+ratio_x*i][ys[me]+j*ratio_y][zs[me]+k],reduced_buffer[i][j][0]);
          x[xs[me]+ratio_x*i][ys[me]+j*ratio_y][zs[me]+k]=reduced_buffer[i][j][0]-x[xs[me]+ratio_x*i][ys[me]+j*ratio_y][zs[me]+k];
