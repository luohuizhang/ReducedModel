...
int main(int argc, char *argv[])
{
	...
	xfinal_delta = malloc(size_x*size_y*size_z*sizeof(*xfinal));
	...
	buffer_all = malloc(xcell*ycell*sizeof(*buffer_all));
	...
	xtemp_delta = malloc(xcell*ycell*zcell*sizeof(*xtemp_delta));
...
for(i=0;i<nproc;i++){
if(me!=i)
		MPI_Send(xyz_index,4,MPI_INT,i,1,comm);    
}

                if(xyz_index[2]==z_domains/2){
	for(i=0;i<nproc;i++){
                if(i==me)
                   continue;
		MPI_Recv(xyz_index_recv,4,MPI_INT,i,1,comm,MPI_STATUS_IGNORE);
                if(xyz_index[0]==xyz_index_recv[0]&&xyz_index[1]==xyz_index_recv[1])
                {  proc_map[index][3]=i;
 ...
 for(i=0;i<nproc;i++){
                if(me==i)
                   continue;
                MPI_Recv(xyz_index_recv,4,MPI_INT,i,1,comm,MPI_STATUS_IGNORE);
		if(xyz_index[0]==xyz_index_recv[0]&&xyz_index[1]==xyz_index_recv[1]&&xyz_index_recv[2]==z_domains/2)
		{ proc_base=xyz_index_recv[3];
		}
	...
	{  
		...
		if(xyz_index[2]==z_domains/2){
                        for(i=0;i<index;i++)
                                MPI_Send(&x0[xs[me]][ys[me]][zs[me]],1,matrix_reduce2full,proc_map[i][3],2,MPI_COMM_WORLD);
                       // printf("%d\n",me);
}
	/*if(xyz_index[2]==z_domains/2&xyz_index[1]==0&&xyz_index[0]==0)
            if(me==32)
                           for(i=0;i<24;i++)
                           for(j=0;j<24;j++)
                        //    for(k=0;k<24;k++) 
                        printf("%f ", &x0[xs[me]+i][ys[me]+j][zs[me]]);
                        if(me==0)
                       printf("\n");
*/
              if(xyz_index[2]==z_domains/2)
             {
              for(i=0;i<xcell;i++)
               for(j=0;j<ycell;j++)
             buffer_reduced[i][j][0] = x0[xs[me]+i][ys[me]+j][zs[me]+zcell/2];
}

              else
		MPI_Recv(&buffer_reduced[0][0][0],xcell*ycell,MPI_DOUBLE,proc_base,2,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
			// printf("Before the linear regen\n");
		regen_linear(x,buffer_reduced, size_x/size_x_reduced, size_y/size_y_reduced,size_z/size_z_reduced,me, xs, xe, ys, ye, zs, ze, xcell, ycell, zcell);
	...
	for(k=zs[me];k<=ze[me];k++)
	{
		...
		for(j=ys[me];j<=ye[me];j++)
		{
			for(m=0;m<=xcell-1;m++){
				...
				xtemp_delta[(l-1)*xcell+(i-1)*xcell*ycell+m] = x[xs[me]+m][j][k];
			}
			l = l+1;
		}
		i = i+1;
	...
	MPI_Gather(xtemp_delta, xcell*ycell*zcell, MPI_DOUBLE, xfinal_delta, xcell*ycell*zcell, MPI_DOUBLE, 0, comm);
	/* Ending time */
				...
				for(j=1;j<=ycell;j++) {
					for(k=0;k<=x_domains-1;k++) {
						for(l=0;l<=xcell-1;l++) {
							...
							fprintf(file_delta,"%15.11f ",xfinal_delta[(j-1)*xcell+l+(y_domains-i)*(z_domains*xcell*ycell*zcell)+k*(y_domains*z_domains*xcell*ycell*zcell)+(p-1)*xcell*ycell]);
						}
					}
					fprintf(file,"\n");
					fprintf(file_delta,"\n");
