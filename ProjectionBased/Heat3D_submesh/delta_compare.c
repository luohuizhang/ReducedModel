...
int main(int argc, char *argv[]){
    ...
    double ad = 0;
    for(int i=0;i<sz;i++){
	diff[i]= ptr2[i%sz1]-ptr1[i];
        if(ptr1[i]!=0)
            rd += fabs((ptr2[i%sz1] - ptr1[i])/ptr1[i]);
        else if(ptr2[i%sz1]!=0)
            rd += 1;  
        ad += fabs((ptr2[i%sz1] - ptr1[i]));
    }
