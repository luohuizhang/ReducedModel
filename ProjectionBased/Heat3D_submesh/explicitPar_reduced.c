...
size_x_full    = iconf_full[0];
	...
	{  
		...
		computeNext1(x0, x, size_tot_x, size_tot_y, size_tot_z, dt, hx, hy, hz, &resLoc, me, xs, ys, zs, xe, ye, ze, k0);
    ...
    //                  for(int j=0;j<24;j++)
      //                    printf("%f ", x0[xs[me]+i][ys[me]+j][zs[me]]);
			...
			for(i=0;i<(nproc_full/nproc_reduced);i++)
				MPI_Send(&x0[xs[me]+(proc_map[i][0]%(x_domains_full/x_domains))*xcell/(x_domains_full/x_domains)][ys[me]+(proc_map[i][1]%(y_domains_full/y_domains))*ycell/(y_domains_full/y_domains)][zs[me]],1,matrix_reduce2full,proc_map[i][3],2,MPI_COMM_WORLD);
			break;
		}
