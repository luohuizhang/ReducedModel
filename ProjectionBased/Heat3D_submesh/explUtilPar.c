...
void regen_linear(double*** x, double*** x0, int ratio_x, int ratio_y, int ratio_z, int me, int* xs, int* xe, int* ys, int* ye, int* zs, int* ze, int xcell, int ycell, int zcell)
{
   /* Index variables */
   int i, j, k;
   ...
   for(k=zs[me];k<=ze[me];k++)
	...
	x[i][j][k]= x[i][j][k]-(kk/ratio_z*temp2+(1-kk/ratio_z)*temp1);
