...
gsl_matrix* pca(const gsl_matrix* data,gsl_matrix* eigenvectors, gsl_matrix* diff,double threshold)
{
	/*
...
int main(int argc, char* argv[])
{
	...
	float *delta =malloc(sizeN*sizeof(float));
	...
	for(int round=0;round<Nrun;round++)
	{
		...
		red_data = pca(&mat.matrix,eigenvectors,diff,Threshold);
		for(i=0;i<rows;i++)
			for(j=0;j<cols;j++)
				delta[j+i*cols]=(float)gsl_matrix_get(diff,i,j);
		//printf("rows = %d, columns = %d \n",rows,cols);
		size_t outSize;
		unsigned char *bytes = SZ_compress(SZ_FLOAT, delta, &outSize, 0, 0, 0,0, rows* cols);
		c_size+=outSize;

		int cols_red=red_data->size2;
		if(cols_red>rows) cols_red=rows;
		for(i=0;i<rows;i++)
			for(j=0;j<cols_red;j++)
				delta[j+i*cols_red]=(float)gsl_matrix_get(red_data,i,j);
		//printf("rows = %d, columns = %d \n",rows,cols);
		signed char *bytes_red = SZ_compress(SZ_FLOAT, delta, &outSize, 0, 0, 0, 0,rows* cols_red);
		c_size+=outSize;

		for(i=0;i<cols;i++)
			for(j=0;j<cols_red;j++)
				delta[j+i*cols_red]=(float)gsl_matrix_get(eigenvectors,i,j);
		//printf("rows = %d, columns = %d \n",rows,cols);
		signed char *bytes_eigenvectors = SZ_compress(SZ_FLOAT, delta, &outSize, 0, 0, 0, 0,cols* cols_red);
		c_size+=outSize;
